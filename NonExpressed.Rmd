...
Data was loaded from the generatino of Figure 4, and expression values were converted to logical variables using the `R` command `is.finite()`.
...
- $\gamma_j$ for $j$ = {1, 2, ..., 20} represents any effects on the probability of expression due each of the 20 combinations of TE and genmoic region
- $y_{ij}$ = logit($\pi_{ij}$), where $\pi_{ij}$ is the probablity of expression for a gene within Tissue $i$ with TEs defined by $j$
