---
title: "Figure 4 (Omitting CDS Regions)"
...
elements <- grep("IDs", colnames(tpmData), value = TRUE) %>% 
  ...
  lapply(unique) %>%
  lapply(sort)
elements$region <- grep("cds", elements$region, value = TRUE, invert = TRUE)
...
```{r}
confIntLen <- list(bootAluUtr3, bootAluUtr5, bootAluProx, 
                   bootL1Utr3, bootL1Utr5, bootL1Prox,
                   bootL2Utr3, bootL2Utr5, bootL2Prox, 
                   bootLTRUtr3, bootLTRUtr5, bootLTRProx,
                   bootMirUtr3, bootMirUtr5, bootMirProx) %>%
  lapply(extract, "logLength") %>%
  lapply(getCI, alpha = 0.05, combined = TRUE) %>%
  bind_rows %>%
  mutate(Region = rep(c("3'UTR", "5'UTR","Proximal Promoter"), 
                      times = nElements),
...
confIntLen %>%
  ...
  labs(x = "95% CI",
       y = c(),
       title = "Difference in log(Length) between samples")
```
...
confIntCombined <- list(bootAluUtr3, bootAluUtr5, bootAluProx, 
                        bootL1Utr3, bootL1Utr5, bootL1Prox,
                        bootL2Utr3, bootL2Utr5, bootL2Prox, 
                        bootLTRUtr3, bootLTRUtr5, bootLTRProx, 
                        bootMirUtr3, bootMirUtr5, bootMirProx) %>%
  ...
  mutate(TE = rep(elements$type, each = nRegions),
         Region = rep(c("3'UTR", "5'UTR", "Proximal Promoter"),
                      times = nElements),
...
confIntTissues <- list(bootAluUtr3, bootAluUtr5, bootAluProx, 
                        bootL1Utr3, bootL1Utr5, bootL1Prox, 
                        bootL2Utr3, bootL2Utr5, bootL2Prox, 
                        bootLTRUtr3, bootLTRUtr5, bootLTRProx, 
                        bootMirUtr3, bootMirUtr5, bootMirProx) %>%
  ...
  mutate(TE = rep(elements$type, 
         ...
         Region = c("3'UTR", "5'UTR", "Proximal Promoter") %>%
           rep(times =nElements) %>%
           rep(each = nTissues),
         Sig = sign(lower) == sign(upper))
```

```{r, plotConfIntTissues, fig.height=8, fig.width=10}
confIntTissues %>%
