...
The data and packages were loaded as beforehand, and the TPM values were once again scaled for length.
...
\begin{itemize}
  \item \textit{alu} elements do not appear to be contributing to reduced gene expression ($P_{adj} = 1$)
  ...
  \item LTR elements seem to have a reduced impact on gene expression in the Brain ($P_{adj} = 2.08E-03$), where the reduction of gene expression only appears to be about a half of what would be expected in Adipose Tissue.
