...
\FloatBarrier
\section{Analysis of Transposable Elements Given Regulatory Elements}
...
<<plotTEgivRE, echo=FALSE, fig.height=4, fig.cap='The estimated probabilities of a base being a Regulatory Element with each genomic region, and the estimated probabilities of a base within each Regulatory Element being a Transposable Element. Error bars indicate one standard error.'>>=
fs <- 8
p1 <- fig2 %>% 
  group_by(Region, RE) %>% 
  summarise(n = n(),
