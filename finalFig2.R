...
p2$theme$strip.text$size <- fs 
p2$theme$axis.text.y$size <- fs
...
grid.newpage()
vp1 <- viewport(x=0.072, y=0.52, w=0.928, h=0.48, just=c("left","bottom"))
vp2 <- viewport(x= 0, y=0, w=1, h=0.5, just=c("left","bottom"))
#pushViewport(vp1)
print(p1, vp=vp1)
print(p2, vp=vp2)
grid.text("A", x = 0.03, y = 0.96, default.units = "npc", gp = gpar(fontsize = fs + 4))
grid.text("B", x = 0.03, y = 0.49, default.units = "npc", gp = gpar(fontsize = fs + 4))
dev.off()
