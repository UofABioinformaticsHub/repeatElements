...
expSum <- expGlm %>%
  ...
  set_names(c("Estimate", "Std.Error", "W", "P")) %>%
  as.data.frame() %>%
...
expSum %>%
  ...
  dplyr::select(Symbol, Coefficient, Estimate, Std.Error, W, P, Sig) %>%
  mutate(P = format(P, digits = 3, scientific = TRUE)) %>%
  kable(digits = 4,
        col.names = c("Model Coefficient", "Term", "Estimate", "SE", "W", "P", " "))
```
...
fig4a <- expSum %>%
  mutate(Symbol = c(paste0("$\\alpha_", 1:6, "$"),
                    ...
                    paste0("$\\gamma_{", 1:20, "}$")),
         Coefficient = gsub("Tissue(.+)", "\\1 Tissue", Coefficient)) %>% 
  dplyr::select(Symbol, Coefficient, Estimate, W, P) %>%
  ...
  ggplot(aes(x = Region, y = TE, fill = Estimate)) +
  geom_raster() +
...
confIntCombined <- list(bootAluUtr3, bootAluUtr5, bootAluProx, bootAluCDS,
  ...
  mutate(TE = rep(elements$type, each = nRegions),
         Region = rep(c("3'UTR", "5'UTR", "Proximal\nPromoter", "CDS"),
                      times = nElements),
         Region = factor(Region,levels = regFactor),
