...
\begin{itemize}
  ...
  \item In Liver L2s appear to be associated with upregulation
  \item In Liver, the repressive effects of LTRs appear to be reversed. Likewise for Skeletal Muscle
  \item In Liver, mirs appear to be associated with upregulation.
\end{itemize}

The interpretation of the above needs careful thought. 
All interaction terms between tissue and an element are in reference to the baseline, and indictate that the observed effects exceed what would be expected if no interaction was observed.
...
Both of these tissues had the highest proportions overall of undetecable genes (Table \ref{tab:notExp}).
...
The negative binomial approach effectively separates the not-expressed and the expressed genes, in an somewhat analagous method to the k-means approach.
...
\begin{itemize}
  ...
  \item L1 elements were inconsisent in their effects, with a potential bias towards genes not being expressed, most strongly observed in Skeletal Muscle
  \item L2 elements showed evidence of an increased probability of expression in Skeletal Muscle, Kidney and Brain. A similar bias was also seen in the other tissues, whilst not formally acheiving significance
...
<<plotCountConfInt, echo=FALSE, fig.height=6, fig.cap="95\\% Confidence Intervals on the logit scale for the tissue-specific effects on the probability of expression. All values are in comparison to the tissue sepcific baseline probability of expression for genes with no TEs, and all intervals used a Bonferroni-adjusted critical value. The zero-line, indicative of no effect is shown as a dotted grey line.">>=
countConfInt %>%
  ggplot(aes(x = Estimate, y = Tissue)) +
  geom_point(size = 3) +
  geom_errorbarh(aes(xmin = lower, xmax = upper), 
  ...
  geom_vline(x = 0, colour = "grey50", linetype=2) +
  theme_bw() +
  labs(x = "Change on expression from baseline", 
       y = "Tissue") + 
...
\begin{itemize}
  \item alu elements showed a bias towards upregulation in Adipose Tissue, Kidney, Liver and Testes, although this was clearly not significant. 
  ...
  \item mir elements also had a significant inhibitory influene in all tissues except Liver, where they appeared to have a positive influence on gene expression.
\end{itemize}
...
In this table, all parameter estimates for liver had noticably higher standard errors, and the overdispersion parameter $\theta$ was also considerably lower, indicating greater overdispersion.
...
This represents less than 20\% of the occurences of each element (Table \ref{tab:singleTEs}), and may be highly unrepresentative on a genome-wide scale.
<<sampSize, echo=FALSE, include=TRUE, results='asis'>>=
singleData %>% 
  distinct(ID) %>% 
  count_("TEtype") %>%
  rename(`Element Type` = TEtype,
         `Number Single TE Genes` = n) %>%
  mutate(`Total TE Genes` = c(none=nrow(filter(tpmData, !alu, !L1, !L2, !LTR, !mir)), 
                   tpmData %>% 
                     select(one_of(elements$type)) %>% 
                     colSums)) %>%
  xtable(digits = 0,
         caption = "Sample sizes for TEs appearing in isolation and in total.",
         label = "tab:singleTEs") %>%
  print(include.rownames = FALSE,
        caption.placement = "top")
@
...
\FloatBarrier
\section{Preparation for Using the Boostrap Approach}
As noted above, the excessive overdispersion observed in the Liver sample may have had an impact of the model fitting stages.
As an alternative to this approach, the sets of genes with a single element were boostrapped against those genes with no elements, in a Tissue specific manner.


% <<blah, eval=FALSE, echo=FALSE>>=
% tissues %>%
%   capwords %>%
%   sapply(function(x){singleTissueNBFits[[x]]$coefficients$zero})
% @
% 
% 
% Prior to fitting, however, the zero-inflation model was fitted across al tissues as a spearate logistic regression model.
% Prediction intervals for the probability of expression for each element type were generated across tissues, using a hypothetical gene of length 2.7kb, which is close to the median gene length observed in the data.
% 
% <<alLogit>>=
% allLogit <- glm(as.logical(rawTPM) ~ 0+ Tissue*TEtype + Tissue*log(length), data = singleData, family=binomial)
% @
% 
% <<predInt>>=
% newData <- data_frame(Tissue = rep(capwords(tissues), each=length(elements$type)+1),
%                       TEtype = rep(c("none", elements$type), times = length(tissues)),
%                       length = 2700)
% critVal <- abs(qnorm(0.05/nrow(newData)))
% preds <- predict(allLogit, newdata = newData, se.fit = TRUE)[1:2] %>%
%   as.data.frame %>%
%   bind_cols(newData) %>%
%   select(Tissue, TEtype, fit, se.fit) %>%
%   mutate(prob = inv.logit(fit),
%          lower = inv.logit(fit - critVal*se.fit),
%          upper = inv.logit(fit + critVal*se.fit))
% @
% 
% <<plotPredInt>>=
% preds %>% 
%   ggplot(aes(x = prob, y = TEtype)) +
%   geom_point() +
%   geom_errorbarh(aes(xmin = lower, xmax = upper)) + 
%   theme_bw() +
...
% 
% 
% The main thing I noticed in the above is that the impact of length varies between tissues.
% Does this make biological sense, or is it an artefact of the different TPM distributions across tissues? \\
% \\
% Re-fitting the model incorporating this possibility gives a better model, as determined by a likelihood ratio test
% <<reFit, cache=TRUE>>=
% fitSingleElementsLengthInt <- hurdle(rawTPM ~ 0 + Tissue*TEtype + log(length)*Tissue | 
%                                    0 + Tissue + TEtype + log(length),
%                                data = singleData, dist = "negbin")
% lrtest(fitSingleElementsLengthInt , fitSingleElementsNoInt )
% @
% 
% Inspection of the cofficients for this model no revealed the Liver \& Skeletal Muscle effects for each TE were significantly different, implying this may possibly be due to the tissue specific TPM distributions.
% 
% \section{Analysis using a bootstrap approach}
% Given these findings above, we need to explore using a bootstrap, but this time we can exclude the not expressed genes and use the mean logTPM values.
