...
tpmData <- read.delim(tpmFile, sep="\t", stringsAsFactors = FALSE) %>% 
  select(ID, length, effective_length, contains("TPM"), contains("utr5")) %>%
  tbl_df
...
<<plotLengthVsTPM, fig.height=8, fig.cap="Raw TPM counts plotted against gene length.", cache=TRUE, dependson='normalisedTPM'>>=
...
<<plotLengthVsnormalisedTPM, fig.height=8, fig.cap="normalised TPM counts plotted against gene length. Note the differing patterns for points associated with the longest gene in Skeletal Muscle. These represent transcripts of the gene TTN which is only expressed in muscle tissue.", cache=TRUE, dependson='normalisedTPM'>>=
...
\section{Looking at the individual Elements}
...
indLm <- lm(normalisedTPM ~ 0 + length +
              Tissue*(alu_utr5_IDs + L1_utr5_IDs + LTR_utr5_IDs ),
   data = indData) %>%
  step
@


<<tabInd, echo=FALSE, results='asis'>>=
summary(indLm) %>% 
  coef %>% 
  as.data.frame %>% 
  add_rownames("Coefficient") %>% 
  ...
  select(-contains("Std")) %>%
  rename(T = `t value`) %>%
  xtable(caption = "Model Coefficients for the effect of the individual elements across all tissues. All $p$ values are adjusted using Bonferroni's method",
         label = "tab:lmTPM", 
